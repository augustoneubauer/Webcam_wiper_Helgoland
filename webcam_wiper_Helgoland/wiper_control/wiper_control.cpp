...
/*
Project: UW Webcam-wiper HELGOLAND Control Software
...
int DisableDevice(DWORD& pErrorCode, long new_targetPosition, long shaft_speed);
...
bool DrivePosition(DWORD& pErrorCode, long new_targetPosition, long shaft_speed)
{
    ...
    // Section to count time needed to complete the displacement. This section is implemented to ensure that the motor is going to be disable after a certain time (time of
    // displacement) to avoid humming noise (if motor is not disabled properly it keeps forcing to move and noise can be noticed)
    float time1;
...
/********************************************************************
    Close Device: Shuts down the operation
*********************************************************************/
int CloseDevice(DWORD& pErrorCode)
{
    int lResult = MMC_FAILED;
...
int main(int argc, char* argv[])
{
    ...
    if (argc == 4)
    {
        ...
        {
            new_targetPosition = displacement * 231 * 6 * (100 / 25); //Mechanical gear ratio for the webcam wiper system
        ...
        {
            new_targetPosition = displacement * ((231 * 6 * 4) / 360); //Mechanical gear ratio for the webcam wiper system
